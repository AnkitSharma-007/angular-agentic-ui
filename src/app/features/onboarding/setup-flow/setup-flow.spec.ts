...
import { OnboardingSetupFlowComponent } from './setup-flow';
...
interface SetupInternals {
  readonly model: { update: (fn: (m: SetupForm) => SetupForm) => void };
...
function patch(inst: SetupInternals, next: Partial<SetupForm>): void {
  inst.model.update((m) => ({ ...m, ...next }));
}

describe('OnboardingSetupFlowComponent', () => {
  beforeEach(() => {
  ...
  it('canTest is false until a key is entered', async () => {
    const fixture = TestBed.createComponent(OnboardingSetupFlowComponent);
    await fixture.whenStable();
    const inst = fixture.componentInstance as unknown as SetupInternals;
